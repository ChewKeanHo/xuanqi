...
views_html_head_write_page_metadata() {
        #____path_dest="$1"
        #____title="$2"
        #____description="$3"
        #____keywords="$4"
        ...
        *)
        ...
        # execute
        printf -- "%s" "\
$(views_html_get_indent "${5:-1}")<title>${2}</title>
$(views_html_get_indent "${5:-1}")<meta name='description' content='${3}' />
$(views_html_get_indent "${5:-1}")<meta name='keywords' content='${4}' />
" >> "${1}.tmp"
        if [ $? -ne 0 ]; then
                return 1
        fi
