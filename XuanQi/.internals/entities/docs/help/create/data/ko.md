...
`create data` 명령줄 인터페이스(CLI)는 프로젝트 전체에서 사용 가능
...
* `[RELATIVE_PATH]`
  * **필수**
  * `$PROJECT_PATH_ROOT`부터의 상대 파일 경로로, `.conf` 파일 확장자
    로 끝나야 합니다.
  * `.conf` 파일 확장자가 제공되지 않으면 XuanQi가 내부적으로 추가합니다.
  * 파일 이름은 사용자의 재량에 따릅니다. XuanQi에 영향을 주지 않습니다.
* `[KEY]`
  * **필수**
  ...
  * 값이 고유한지 확인하십시오. 그렇지 않으면 여러 구성 파일에서 동일
    한 키를 사용할 경우
    예측할 수 없는 결과(마지막으로 구문 분석된 파일이 이전 파일을 덮어
    씀)가 발생할 수 있습니다.
* `[VALUE]`
  * **선택 사항**
  * `key:value` 항목의 `value` 레이블입니다.
  * 비워 두면 샘플 텍스트로 대체되며, 나중에 파일을 업데이트할 수 있
    습니다.
...
보안상의 이유로 XuanQi는 데이터 파일을 `$PROJECT_PATH_ROOT` 내에서만 생
성할 수 있도록합니다. 그렇지 않으면 오류가 발생하고 아무 작업도 수행되
지 않습니다.
