...
# Specifications:
#       - To trim a given string's whitespaces at most right until it meets
#         with a non-whitespace characters. The left side remains in tact.
...
#           2.       XuanQi_Strings_Trim_Whitespace_Right "  Sample Text     " \
#           3. )"
...
#               - always 0.
XuanQi_Strings_Trim_Whitespace_Right() {
        #____content="$1"
        ...
        # execute
        interactors_strings_trim_whitespace_right "$1"
        if [ $? -ne 0 ]; then
                return 1
        fi
